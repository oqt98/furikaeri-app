...
初回リリースでは `reviews(user_id, review_date)` を一意にしていません。

理由:

- 今のアプリは 1 日 1 件を強制していない
- import 処理や既存データとの整合を壊したくない
- まずは今の `ReviewItem` と対応しやすいことを優先したい

つまり、DB でも「同じ日に複数件」を許容します。
もし将来 1 日 1 件にしたくなったら、その時点でアプリ仕様と合わせて unique 制約を追加する方が安全です。
